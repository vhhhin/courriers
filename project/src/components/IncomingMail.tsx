...
export const IncomingMail = ({ isOpen, onClose, language }: IncomingMailProps) => {
  ...
  const handleExportPDF = () => {
    exportCourriersPDF(mails, 'arrive');
  };

  const handlePrint = () => {
    const printWindow = window.open('', '_blank');
    const tableContent = mails.map(({ number, date, sender, subject, reference, recipient }) => `
      <tr>
        <td>${number}</td>
